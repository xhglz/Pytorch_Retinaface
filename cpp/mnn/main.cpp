...
int main(int argc, char **argv)
{
  ...
  string modelpath = "models/FaceDetector_quant.mnn"; //model path
  ...
  //process each image
  for (size_t i = 0; i < list_test_vec.size(); i++)
  {
    cv::Mat bgr;
    ...
    for (size_t i = 0; i < faces.size(); i++)
    {
      ...
      cv::putText(bgr, score, cv::Point(face.rect.x, face.rect.y), cv::FONT_HERSHEY_DUPLEX, 0.6, cv::Scalar(0,255,255));
    }
